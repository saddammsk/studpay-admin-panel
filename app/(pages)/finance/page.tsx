...
export default function GlobalFinancing() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      <aside
        className={`flex flex-col bg-white border-r border-gray-100 transition-all duration-300 ${collapsed ? "w-14" : "w-56"} shrink-0`}
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-800">StudPay</span>
            </div>
          )}
          <button
            ...
            <div key={group.title}>
              ...
              <div className="space-y-0.5">
                {group.items.map((item) => (
                  ...
                  >
                    <span className={`shrink-0 ${item.active ? "text-white" : ""}`}>{item.icon}</span>
        ...
        <div className="border-t border-gray-100 px-2 py-3 space-y-0.5">
          {[
            ...
            <button
              key={i.label}
              className="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-all"
            >
              <span className="shrink-0">{i.icon}</span>
              {!collapsed && <span className="text-xs font-medium">{i.label}</span>}
            </button>
          ))}
          <div className="flex items-center gap-2 px-2 py-1.5 mt-1">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-gray-400 truncate">admin@studpay.com</p>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-100 px-6 py-3.5 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-base font-bold text-gray-900">Global Financing</h1>
            <p className="text-xs text-gray-400 mt-0.5">Manage loan applications and pipeline</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-1.5 bg-blue-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-blue-700 transition-colors">
              <Plus size={14} />
              New Application
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                <p className="text-xs text-gray-500 font-medium">Total Loan Volume</p>
                <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center">
                  <BookOpen size={15} className="text-blue-500" />
                </div>
              </div>
              <p className="text-2xl font-bold text-gray-900 tracking-tight">€2,450,000</p>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-gray-900 tracking-tight">12</p>
              <p className="text-xs text-gray-400 mt-1">Awaiting review</p>
              <div className="flex items-center gap-1 mt-2">
                <TrendingUp size={12} className="text-green-500" />
                <span className="text-xs font-semibold text-green-600">3 new today</span>
                <span className="text-xs text-gray-400">vs last month</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-gray-900 tracking-tight">2.4 days</p>
              <p className="text-xs text-gray-400 mt-1">Application to approval</p>
              <div className="flex items-center gap-1 mt-2">
                <TrendingUp size={12} className="text-green-500" />
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                <p className="text-xs text-gray-500 font-medium">Repayment Health</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div className="flex items-center border-b border-gray-100 px-4">
              {[
                { key: "loan" as const, label: "Loan Ledger", icon: <BookOpen size={13} /> },
                { key: "partner" as const, label: "Partner Management", icon: <Handshake size={13} /> },
                { key: "api" as const, label: "API Console", icon: <Terminal size={13} /> },
              ].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs text-gray-400">{loanData.length} records</span>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100">
                      {["DOSSIER ID", "APPLICANT", "AMOUNT", "INSTRUCTING BANK", "CASE MANAGER", "SLA (DAYS)", "STATUS", "AI SCORE"].map(
                        (h) => (
                          <th key={h} className="text-left text-[10px] font-semibold text-gray-400 tracking-wider pb-2.5 pr-4">
                            {h}
                          </th>
                        )
                      )}
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {loanData.map((row) => (
                      <tr key={row.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="py-3 pr-4">
                          <span className="text-xs font-medium text-gray-700">{row.applicant}</span>
                        </td>
                        <td className="py-3 pr-4">
                          <span className="text-xs font-semibold text-gray-800">{row.amount}</span>
                        </td>
                        <td className="py-3 pr-4">
                          <span className="text-xs text-gray-600">{row.bank}</span>
                        </td>
                        <td className="py-3 pr-4">
                          <p className="text-xs font-medium text-gray-700">{row.manager}</p>
                          <p className="text-[10px] text-gray-400">{row.phone}</p>
                        </td>
                        <td className="py-3 pr-4">
                          <span className={`text-xs font-bold ${row.slaColor}`}>{row.sla}d</span>
                        </td>
                        <td className="py-3 pr-4">
                          <StatusBadge status={row.status} />
                        </td>
                        <td className="py-3">
                          <ScoreBadge score={row.score} colorClass={row.scoreColor} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
